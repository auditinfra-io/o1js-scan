...
class SemanticFacts:
    ...
    def witness(self, method: str, parameter_index: int) -> Tuple[Optional[EffectFact], str]:
        """Return the first reachable effect and constraint level for a parameter."""
        summary = self._summaries.get(method, _Summary({}, {}))
    ...
    def _compute_one(self, meth: object) -> _Summary:
        ...
        for match in _CALL.finditer(body):
            callee = self._summaries.get(match.group(1))
            if callee is None:
                continue
            ...
            for callee_i, fact in callee.effects.items():
                if callee_i < len(args):
                    for ident in _IDENT.findall(args[callee_i]):
                        for caller_i in deps.get(ident, set()):
