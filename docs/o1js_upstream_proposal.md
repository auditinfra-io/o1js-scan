# Proposal: run `o1js-scan` in the o1js repository

This is a ready-to-send proposal for the o1js maintainers. It deliberately asks
for a small, reversible first-party integration rather than claiming that a
community-maintained scanner is already an official o1js security tool.
...
- Confirm that the merged Community Packages entry was based on v0.13.0. The
  comparison above uses the version present when the upstream listing PR was
  opened; if maintainers reviewed a later commit, adjust the baseline.
- Check o1js's current contribution and RFC process before opening anything.
- Keep the initial ask advisory. Requesting a required security gate, vendored
  code, and transfer of governance in one step would make evaluation harder.
- Be candid that the npm wrapper requires Python 3.8+ on `PATH`. The analyzer
  itself has no third-party Python dependencies, but npm alone does not provide
  the interpreter.
- Do not quote raw findings from upstream tests as vulnerabilities. The upstream
  compatibility canary intentionally includes tests and examples to exercise
  syntax; its findings are not a vulnerability budget.
